...
# Estimate time functions: Probably wrong now
...
class FeedForwardNN():
    ...
    def build_net(self,
        ...
        # Size of objective and input size
        self.goallength=goallength
        self.inputlength=inputlength

        # Number of instruction types and symbols of one logic type
    ...
    def train(self, path_rai, model_dir, saveToFile=True):
        ...
        # tmp directory: Save current network
        if saveToFile and not(os.path.exists(path_rai+'/logs/'+self.timestamp+'/tmp')):
        ...
        if saveToFile:
            ...
            # Save params and copty data set info
...
class ClassifierMixed():
    ...
    def reshapeInput(self, path_rai, model_dir):
        ...
        # Expand data of length 4 with some samples (30%) of length 3
    ...
    def train(self, path_rai, model_dir, saveToFile=True):
        ...
        if saveToFile:
            ...
            # Save parameters and data set information
            with open(path_rai+'/logs/'+self.timestamp+'/params.txt', 'w') as f:
        ...
        modelInstructHist=self.modelInstruct.fit(x={"goal":InstrList[4][0][:,0:1,:self.goallength],"state":InstrList[4][0][:,:,self.goallength:] },
                                                ...
                                                epochs=self.epochs_inst, shuffle=True, verbose=0, validation_split=self.val_split,

...
DecisionRule place {
  X, Y, Z,
  { (grasped X Y) (table Z) (held Y) (on Y Z)! (ontop Y Z)! (same Y Z)! (infeasible Y Z)!}# (object Z)!
...
#DecisionRule place {
#  X, Y, Z,
#  { (grasped X Y) (table Z) (held Y) (on Y Z)! (ontop Y Z)! (same Y Z)! (on Z ANY)! (object Z)}
#  { (grasped X Y)! (busy X)! (busy Y)! (held Y)! # logic only
#    (stable ANY Y)! (touch X Y)! # NLP predicates
#    (on Z Y) (above Y Z) (stableOn Z Y) tmp(touch X Y) tmp(touch Y Z)
#    (INFEASIBLE grasp ANY Y)! block(INFEASIBLE grasp ANY Y)
...
Rule{
  A, B, C
  { (held A) (object B) (table B) (on B C)}
  { (infeasible A B)}
}
